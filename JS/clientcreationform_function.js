...
document.addEventListener('DOMContentLoaded', function() {
    ...
    clientCreationForm.addEventListener('submit', async (e) => {
        ...
        const requiredFields = [
            ...
            { name: 'Barangay', value: data.barangay, id: 'barangay' },
            { name: 'Street Address', value: data.streetAddress, id: 'streetAddress' },
            { name: 'Phone Number', value: data.phoneNumber, id: 'phoneNumber' },
            { name: 'Employment Status', value: data.employmentStatus, id: 'employmentStatus' },
            { name: 'Occupation/Position', value: data.occupationPosition, id: 'occupationPosition' },
            { name: 'Years in Job', value: data.yearsInJob, id: 'yearsInJob' },
            { name: 'Income/Salary', value: data.incomeSalary, id: 'incomeSalary' },
        ];
        ...
        for (const field of requiredFields) {
            if (!field.value || field.value.trim() === '') {
                ...
                const description = `FAILED: Client creation failed due to missing required field: ${field.name}.`;
                logUserAction(actionType, description);
        ...
        if (!barangayClearanceCheck.checked || !hasValidIdCheck.checked) {
            ...
            const description = 'FAILED: Client creation failed because BOTH required documents (Barangay Clearance/Certificate and Valid ID) were not checked.';
            logUserAction(actionType, description);
            return; 
        }
        ...
        if (hasValidIdCheck.checked && (!validIdTypeSelect.value || validIdTypeSelect.value === '')) {
            ...
            const description = 'FAILED: Client creation failed because Valid ID was checked but Valid ID Type was not selected.';
            logUserAction(actionType, description);
            return;
        }
