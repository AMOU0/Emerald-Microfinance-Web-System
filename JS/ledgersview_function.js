...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const returnButton = document.querySelector('.return-btn'); // Get the return button
    ...
    if (logoutButton) {
      logoutButton.addEventListener('click', function() {
        // Log the pending logout click (The server-side check_logout.php will log the SUCCESSFUL logout event)
        logUserAction('LOGOUT_CLICK', 'User clicked the Logout button to terminate session.', 'SESSION', null);
        window.location.href = 'PHP/check_logout.php'; 
      });
    }

    // Attach the logging function to the return button
    if (returnButton) {
...
// NEW (Correctly looking for 'client_id')
...
async function fetchAndDisplayClientDetails() {
    ...
    try {
        ...
        // Note: The HTML has two elements with the same ID 'income-collateral-dd' and 'income-collateral-dd-2'.
        // For correctness, I'm assuming 'income-collateral-dd' should show 'Employment / Income'
        // and 'income-collateral-dd-2' should show 'Collateral' based on the surrounding <dt> tags.

        // Corrected population based on HTML structure
        ...
        // You would have a separate function here to fetch and display loan details (table)
        // For example: fetchAndDisplayLoanDetails(clientId);
        ...
        .then(data => {
            ...
            document.querySelectorAll('.clickable-loan-row').forEach(row => {
                ...
                row.addEventListener('click', function() {
                    ...
                    if (loanId) {
                        // **NEW ACTION**: Call the modal function instead of navigation
...
const targetClientId = urlParams.get('client_id'); // 🎯 FIX: Changed to client_id
    if (targetClientId) {
...
function showLoanDetailsModal(loanId) {
    ...
    fetch(`PHP/ledgersviewfetchloan_handler.php?loanId=${loanId}`)
        .then(response => {
            if (!response.ok) {
                throw new Error(`HTTP error! Status: ${response.status}`);
            }
...
// ------------------------------------------------------------------
// CLOSURE AND EVENT LISTENERS (Remains Unchanged)
// ------------------------------------------------------------------
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (modal) {
        window.addEventListener('click', (event) => {
            if (event.target === modal) {
                // Log modal closure via outside click
    ...
    // NOTE: The generic document.body.addEventListener('click', ...) for [data-loan-id]
    // has been removed as the more specific listener inside loadLoanDetailsFromDatabase now handles
    // the logic and logging for dynamic rows.
});